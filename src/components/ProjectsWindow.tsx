  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-4">
        <h2 className="font-bold text-sm">Current Projects</h2>
        <button 
      ...
      {showSuggestionForm && (
        <div className="mb-4 p-3 border border-gray-400 bg-gray-100">
          <form onSubmit={handleSubmitSuggestion}>
            <label className="block text-xs mb-1">Project Suggestion:</label>
            <input
              type="text"
              value={suggestion}
            ...
            <div className="flex gap-2 mt-2">
              <button type="submit" className="retro-button text-xs">Submit</button>
              <button 
                type="button" 
      ...
      <div className="flex-1 overflow-auto">
        {loading && <div className="text-xs text-gray-500 p-2">Loading projects...</div>}
        {projects.map((project) => {
          const isExpanded = expandedProjects.has(project.id);
          return (
            <div key={project.id} className="mb-4 p-3 border border-gray-400 bg-white">
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-start gap-2 flex-1">
                  {project.details && (
                    <button
                      onClick={() => toggleExpanded(project.id)}
                      className="text-gray-600 hover:text-gray-800 mt-0.5"
                    >
              ...
              {isExpanded && project.details && (
                <div className="mb-2 p-2 bg-gray-50 border border-gray-200 text-xs">
